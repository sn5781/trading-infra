...
def main():
    ...
    cds=ib.reqContractDetails(Future(symbol='CL',exchange='NYMEX'))
    valid=sorted([cd.contract for cd in cds if cd.contract.lastTradeDateOrContractMonth>=ts],
                 key=lambda c: c.lastTradeDateOrContractMonth)
    if len(valid)<2: print('need 2+ CL',file=sys.stderr); ib.disconnect(); sys.exit(1)
    res={}
    for lbl,c in [('front',valid[0]),('next',valid[1])]:
